...
class _XMLInjectionSet(object):
    ...
    def apply(self, strain, detector_name, f_lower=None, distance_scale=1,
        ...
        for j, inj in enumerate(injections):
            print("Injection %s/%s", j, len(injections))
    ...
    def make_strain_from_inj_object(self, inj, delta_t, detector_name,
        ...
        f_l = 20
        dur = spa_length_in_time(mass1=inj.mass1, mass2=inj.mass2, f_lower=f_l, phase_order=-1)
        while dur > mlen:
            f_l += 1
            dur = spa_length_in_time(mass1=inj.mass1, mass2=inj.mass2, f_lower=f_l, phase_order=-1)

        import logging
        logging.info("We picked an flow of %s for this injection: %s, %s, %s, %s", f_l, inj.waveform, dur, inj.mass1, inj.mass2)
        inj.f_lower = f_l
