...
class DistMarg():
    ...
    def marginalize_loglr(self, sh_total, hh_total,
        ...
        # The per-sample vector is what the effective sample size, the
        # inline draw and the adaptive proposal are all built from, so ask
        # for it once and hand it to each. The peak and reconstruction paths
        # do not marginalize a vector here, so adaptation records on the
        # ordinary path only; that is every call the sampler makes.
        ...
        if self._adapting() and not numpy.isscalar(sh_total):
            # shapes the proposal the NEXT call draws from
            self._adapt_record(vector)
    ...
    def current_params(self):
        ...
        # The weights themselves are reset in update(), not here: a draw's
        # weights have to survive the parameters being read again.
        if self._adapting():
