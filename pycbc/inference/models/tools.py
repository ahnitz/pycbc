...
class DistMarg():
    ...
    def premarg_draw(self):
        ...
        # The points came from the sky grid, so the stored factors describe
        # this draw too and are restored alongside the indices that select
        # it. snr_draw clears them for a scalar coalescence time, and this
        # is the only place that puts them back.
    ...
    def snr_draw(self, wfs=None, snrs=None, size=None):
        ...
        if not set_scalar:
            ...
            # marginalizations. The stored factors describe a draw that no
            # longer applies.
