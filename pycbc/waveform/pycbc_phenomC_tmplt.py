...
import numpy
from numpy import sqrt, log, float128, euler_gamma as GAMMA
from scipy.constants import c as C_SI, G as G_SI, parsec
...
MTSUN_SI = MT_sun.value
MRSUN_SI = MR_sun.value
MSUN_SI = M_sun.value
...
def fRD( a, M):
    ...
    f = (C_SI**3.0 / (2.0*numpy.PI*G_SI*M*MSUN_SI)) * (1.5251 - 1.1568*(1.0-a)**0.1292)
    return f
...
def imrphenomc_tmplt(**kwds):
    ...
    distance *= (1.0e6 * parsec / (2. * sqrt(5. / (64.*numpy.PI)) * M * MRSUN_SI * M * MTSUN_SI))
    ...
    pfa3 = -16.0*numpy.PI + (113./3.)*Xi - 38.*eta*Xisum/3.
    pfa4 = (152.93365/5.08032) - 50.*Xi2 + eta*(271.45/5.04 + 1.25*Xiprod) + \
        3085.*eta2/72.
    pfa5 = numpy.PI*(386.45/7.56 - 65.*eta/9.) - \
        Xi*(735.505/2.268 + 130.*eta/9.) + Xisum*(1285.0*eta/8.1 + 170.*eta2/9.) - \
        10.*Xi2*Xi/3. + 10.*eta*Xi*Xiprod
    pfa6 = 11583.231236531/4.694215680 - 640.0*numpy.PI*numpy.PI/3. - \
        6848.0*GAMMA/21. - 684.8*log(64.)/6.3 + \
        eta*(2255.*numpy.PI*numpy.PI/12. - 15737.765635/3.048192) + \
        76.055*eta2/1.728 - (127.825*eta2*eta/1.296) + \
        2920.*numpy.PI*Xi/3. - (175. - 1490.*eta)*Xi2/3. - \
        (1120.*numpy.PI/3. - 1085.*Xi/3.)*eta*Xisum + \
        (269.45*eta/3.36 - 2365.*eta2/6.)*Xiprod

    pfa6log = -6848./63.

    pfa7 = numpy.PI*(770.96675/2.54016 + 378.515*eta/1.512 - 740.45*eta2/7.56) - \
        ...
        560.*numpy.PI*Xi2 + 20.*numpy.PI*eta*Xiprod + \
        Xi2*Xi*(945.55/1.68 - 85.*eta) + Xi*Xiprod*(396.65*eta/1.68 + 255.*eta2)


    xdotaN = 64.*eta/5.
    xdota2 = -7.43/3.36 - 11.*eta/4.
    xdota3 = 4.*numpy.PI - 11.3*Xi/1.2 + 19.*eta*Xisum/6.
    xdota4 = 3.4103/1.8144 + 5*Xi2 + eta*(13.661/2.016 - Xiprod/8.) + 5.9*eta2/1.8
    xdota5 = -numpy.PI*(41.59/6.72 + 189.*eta/8.) - Xi*(31.571/1.008 - 116.5*eta/2.4) + \
          Xisum*(21.863*eta/1.008 - 79.*eta2/6.) - 3*Xi*Xi2/4. + \
          9.*eta*Xi*Xiprod/4.
    xdota6 = 164.47322263/1.39708800 - 17.12*GAMMA/1.05 + \
          16.*numpy.PI*numpy.PI/3 - 8.56*log(16.)/1.05 + \
          eta*(45.1*numpy.PI*numpy.PI/4.8 - 561.98689/2.17728) + \
          5.41*eta2/8.96 - 5.605*eta*eta2/2.592 - 80.*numpy.PI*Xi/3. + \
          eta*Xisum*(20.*numpy.PI/3. - 113.5*Xi/3.6) + \
          Xi2*(64.153/1.008 - 45.7*eta/3.6) - \
          Xiprod*(7.87*eta/1.44 - 30.37*eta2/1.44)

    xdota6log = -856./105.

    xdota7 = -numpy.PI*(4.415/4.032 - 358.675*eta/6.048 - 91.495*eta2/1.512) - \
          ...
          12.*numpy.PI*Xi2 - Xi2*Xi*(150.5/2.4 + eta/8.) + \
          Xi*Xiprod*(10.1*eta/2.4 + 3.*eta2/8.)


    AN = 8.*eta*sqrt(numpy.PI/5.)
    A2 = (-107. + 55.*eta)/42.
    A3 = 2.*numpy.PI - 4.*Xi/3. + 2.*eta*Xisum/3.
    A4 = -2.173/1.512 - eta*(10.69/2.16 - 2.*Xiprod) + 2.047*eta2/1.512
    A5 = -10.7*numpy.PI/2.1 + eta*(3.4*numpy.PI/2.1)

    A5imag = -24.*eta

    A6 = 270.27409/6.46800 - 8.56*GAMMA/1.05 + \
      2.*numpy.PI*numpy.PI/3. + \
      eta*(4.1*numpy.PI*numpy.PI/9.6 - 27.8185/3.3264) - \
      20.261*eta2/2.772 + 11.4635*eta*eta2/9.9792 - \
      4.28*log(16.)/1.05

    A6log = -428./105.

    A6imag = 4.28*numpy.PI/1.05
