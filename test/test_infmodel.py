...
class TestModels(unittest.TestCase):
    ...
    # Models that marginalize over time. They locate the signal from its
    # SNR peak, so they need data that actually contains one, hence they
    # live here rather than with the signal-free noise of
    # TestWaveformErrors. ``a1`` is phase-marginalized, so it is only a
    # valid reference with phase marginalization turned on as well.
    ...
    def marg_time_model(self, nsamples=512):
    ...
    def relative_time_model(self, cls, nsamples=512):
        variable, prior = self.tc_setup()
        static = {k: v for k, v in self.static.items() if k != 'tc'}
        return cls(
            variable, copy.deepcopy(self.data),
    ...
    def test_time_marginalization_is_converged(self):
        ...
        The count has to be set when the model is built; it is read once
        into ``vsamples`` and assigning to the model afterwards does
        nothing.
        """
        for build in (self.marg_time_model,
                      lambda n: self.relative_time_model(
                          models.RelativeTime, n)):
            values = []
            for nsamples in (128, 2048):
                model = build(nsamples)
                model.update(**self.q1)
...
class TestAnalyticModels(unittest.TestCase):
    ...
    def test_normal(self):
        ...
        # a non-default mean/cov, so those arguments are exercised too
        mean, cov = [1., -2.], [4., 0.25]
